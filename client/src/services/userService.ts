...
export const registerUser = async (name: string, password: string) => {
    const registeringUser = await axios.post("http://localhost:3000/user/register", {
        name: name,
        password: password,
    });
...
export const loginUser = async (name: string, password: string) => {
    ...
    localStorage.setItem("userId", loggingInUser.data.userId);
    localStorage.setItem("token", loggingInUser.data.token);

    return loggingInUser.data;
}
